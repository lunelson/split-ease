...
function drawInterpGraph(ctx, x, y, width, height, fx) {
  ...
  // parse last point for [1,1]
  var lastSlope = (1 - prevXY[1])/(1 - prevXY[0]);
  ctx.lineTo(1, 1);
  slopes.push(lastSlope);
...
/**
 * LEVEL 3
 * see this http://codeincomplete.com/posts/javascript-game-foundations-rendering/
 * and the CM episode about drag and drop
 */
...
var graphPending = true;
function renderGraph(width, height, fx, canvas) {
  canvas = canvas || createCanvas(width, height, canvas);
  var ctx = canvas.getContext('2d');

  // put the code for graphing here
...
function renderMaster() {
  ...
  if (graphPending) { renderGraph(width - 80, height - 80, graphFunction, graphCanvas); }
  ctx.drawImage(graphCanvas, 40, 40);

  // do some other stuff that will update all the time
